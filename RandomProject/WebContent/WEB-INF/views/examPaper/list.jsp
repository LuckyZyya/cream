	...
	<form id="edit-form" method="post">
        ...
        <table>
            <tr>
                ...
                <td>
                	<select id="edit-examId" name="examId" class="easyui-combobox easyui-validatebox" panelHeight="auto" style="width:268px" data-options="required:true, missingMessage:'请选择试卷课程'">
		                <c:forEach items="${examList}" var="exam">
			            	<option value="${exam.id}">${exam.name}</option>
		            	</c:forEach>
		            </select>
            ...
            <tr>
                ...
                <td>
                	<select id="edit-studentId" name="studentId" class="easyui-combobox easyui-validatebox" panelHeight="auto" style="width:268px" data-options="required:true, missingMessage:'请选择试卷课程'">
		                <c:forEach items="${studentList}" var="student">
			            	<option value="${student.id}">${student.name}</option>
		            	</c:forEach>
		            </select>
	...
	*/
	$('#data-datagrid').datagrid({
		...
		columns:[[
			...
			{ field:'examDiffcult',title:'组卷难度',width:200,formatter:function(value,index,row){
				if(value == 0) return '简单';
				if(value == 1) return '中等';
				if(value == 2) return '困难';
				return '不限';
			}},
			 {field:'examDistribute',title:'包含知识点',width:200,formatter:function(value,index,row){
					if(value == 1) return '第一章';
					if(value == 2) return '第二章';
					if(value == 3) return '第三章';
					if(value == 4) return '第四章';
					if(value == 5) return '第五章';
					return '综合';
				}},
